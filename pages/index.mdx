...
So, you are probably familiar with Cloudflare's awseome CDN, which allows developers to publish their application's static files(css & javascript) and images to their vast network of over 200 nodes world wide.

Well imagine if you could deploy your code to the same 200+ worldwide servers, **drastically** reducing network latency.

And to take it one step further what if your entire database was also deployed and synconized to the same 200+ worldwide nodes...

Well this is exactly what SonicJs on Cloudflare workers allows you to do! Global users will interact with your application's API with blazingly fast response times.

## How Fast is "Blazingly" Fast?

Let's compare SonicJs to Node with a Postgres daatasbe and Strapi, a popular open source Headless CMS using a Postgres database. 

The intention is to compare SonicJs to a typical Node app running on a server infrastructure setup that is most common amoung product applications.

| Platform      | Average Response Time | Difference |
| ----------- | ----------- | ----------- |
| Strapi      | 460ms       | - |
| Node + Postgres   | 420ms        | 1.1x Faster|
| SonicJs   | 70ms        | 7.3x Faster|



The details of our performance benchmark here is available at [here](/performance-benchmarks). This includes dataset details,
...
If you're interested in contributing to this open-source project or have any thoughts, join our existing Discord (https://discord.gg/nYAWyadw) community.
