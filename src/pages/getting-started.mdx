...
The above script will automatically rename `/wrangler.example.toml` to `/wrangler.toml` and update the 4 required Cloudfalre values:
...
The next thing you will want to do is define your database table(s) and read up on SonicJs's built in caching and [persistance](/persistance) layers.