...
<Row>
  <Col>
    This endpoint confirms the user's email address using the confirmation code and automatically logs them in.
    ...
    ### Return Data
    Redirects to the configured `EMAIL_CONFIRMATION_REDIRECT_URL` with an active session

    ### Auto Login
    After successful email confirmation, the system automatically:
    - Creates a new session for the user
    - Sets the session token in cookies
    - Redirects to the configured redirect URL

    ### Security
    - The confirmation token is single-use and expires after a set period
    - Session creation follows the same security protocols as manual login
    - Session tokens are generated using secure random strings
    - All sessions have configurable expiration times
...
<Properties>
  ...
  <Property name="AUTO_LOGIN_AFTER_EMAIL_CONFIRMATION" type="boolean" default="true">
    Controls whether users are automatically logged in after email confirmation
  </Property>
